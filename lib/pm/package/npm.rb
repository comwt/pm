...
class PM::Package::NPM < PM::Package
  class << self
    include PM::Util
  end
  ...
  def self.default_prefix
    ...
    if npm_prefix.count("\n") > 0
      raise PM::InvalidPackageConfiguration, "`npm prefix -g` returned unexpected output."
    elsif !File.directory?(npm_prefix)
      raise PM::InvalidPackageConfiguration, "`npm prefix -g` returned a non-existent directory"
    end
