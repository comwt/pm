...
#     pkg = PM::Package::Python.new
#     pkg.input("Django")
#
class PM::Package::Python < PM::Package
  ...
  def load_package_info(setup_py)
    ...
    begin
      ...
      raise PM::Util::ProcessFailed, "Python (#{attributes[:python_bin]}) is missing simplejson or json modules."
    end
    ...
    begin
      ...
      raise PM::Util::ProcessFailed, "Python (#{attributes[:python_bin]}) is missing pkg_resources module."
    end

    # Add ./pypm/ to the python library path
    ...
    output = ::Dir.chdir(setup_dir) do
      ...
      setup_cmd = "env PYTHONPATH=#{pylib} #{attributes[:python_bin]} " \
        "setup.py --command-packages=pypm get_metadata --output=#{tmp}"
      ...
      # package. See pm/lib/pm/package/pypm/get_metadata.py for more
      # details.
    ...
    if !attributes[:no_auto_depends?] and attributes[:python_dependencies?]
      metadata["dependencies"].each do |dep|
        ...
        if match.nil?
          ...
          raise PM::InvalidPackageConfiguration, "Invalid dependency '#{dep}'"
        end
