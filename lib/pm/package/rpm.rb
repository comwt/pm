...
class PM::Package::RPM < PM::Package
  ...
  option "--os", "OS", "The operating system to target this rpm for. " \
    "You want to set this to 'linux' if you are using pm on OS X, for example"
  ...
  option "--verbatim-gem-dependencies", :flag,
           "When converting from a gem, leave the old (pm 0.4.x) style " \
  ...
  def architecture
    case @architecture
      ...
      when "all"
        # Translate pm "all" arch to what it means in RPM.
        return "noarch"
      else
        return @architecture
    end
  ...
  # See PM::Package#converted_from
  def converted_from(origin)
    if origin == PM::Package::Gem
      fixed_deps = []
