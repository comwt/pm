...
class PM::Package::Dir < PM::Package
  private
  ...
  def input(path)
    ...
    #   and 'source=dest' in pm work the same as the above rsync
    ...
    begin
      ::Dir.chdir(chdir) do
        begin
          ...
          raise PM::InvalidPackageConfiguration,
  ...
  def clone(source, destination)
    ...
    # pm to recursively (and forever) copy the staging directory by
    # accident (#542).
    if File.expand_path(source) == File.expand_path(::Dir.tmpdir)
      raise PM::InvalidPackageConfiguration,
        "A source directory cannot be the root of your temporary " \
        "directory (#{::Dir.tmpdir}). pm uses the temporary directory " \
        "to stage files during packaging, so this setting would have " \
        "caused pm to loop creating staging directories and copying " \
  ...
  def copy(source, destination)
    ...
    if dstat.nil?
      ...
      raise PM::InvalidPackageConfiguration, "Tried to treat #{readable_path} like a directory, but it's a file!"
    end
