...
class PM::Package::Deb < PM::Package
  ...
  # Take care about the case when we want custom control file but still use pm ...
  ...
  # Get the name of this package. See also PM::Package#name
  #
    ...
    begin
      if !attributes[:deb_no_default_config_files?]
        logger.warn("Debian packaging tools generally labels all files in /etc as config files, " \
                    "as mandated by policy, so pm defaults to this behavior for deb packages. " \
