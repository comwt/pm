require "pm/namespace" # local
require "pm/util" # local
...
# If you want to implement an PM package type, you'll inherit from this.
class PM::Package
  include PM::Util
  ...
  #
  # Iteration can be nil. If nil, the pm package implementation is expected
  ...
  def initialize
    ...
    # that also are obeyed if pm is used programmatically.
  ...
  #
  # For PM::Package::ABC, this returns 'abc'
  def type
    self.class.type
  end # def type
  ...
  # For instance:
  #
  # * for PM::Package::Dir, << expects a path to a directory or files.
  # * for PM::Package::RPM, << expects a path to an rpm.
  #
  ...
  def files
    ...
    # but requires the 'backports' gem (which is used in other places in pm)
  ...
  def edit_file(path)
    editor = ENV['PM_EDITOR'] || ENV['EDITOR'] || 'vi'
  ...
  class << self
    ...
    #
    # Lets us track all known PM::Package subclasses
    def inherited(klass)
      @subclasses ||= {}
    ...
    # This allows packages to define flags for the pm command line
  ...
  def version
    if instance_variable_defined?(:@version) && !@version.nil?
      ...
      # pm command-line tool has been given '-v' or '--version' settings
      # We do this check because the default version is "1.0"
      # on the pm command line.
