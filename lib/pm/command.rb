...
Dir[File.join(File.dirname(__FILE__), "package", "*.rb")].each do |plugin|
  ...
  require "pm/package/#{File.basename(plugin)}"
end


# The main pm command entry point.
class PM::Command < Clamp::Command
  include PM::Util
  ...
  def help(*args)
    ...
    PM::Package.types.each do |name, _|
      lines.push("  - #{name}")
    end
    lines.push("")
    lines.push(super)
  ...
  option "--log", "LEVEL", "Set the log level. Values: #{loglevels.join(", ")}.",
    ...
    val.downcase.tap do |v|
      if !loglevels.include?(v)
        raise PM::Package::InvalidArgument, "Invalid log level, #{v.inspect}. Must be one of: #{loglevels.join(", ")}"
      end
    end
  end # --log
  ...
  option "--template-scripts", :flag,
    ...
    "see the pm wiki: " \
    "https://github.com/comwt/pm/wiki/Script-Templates"
  ...
  option "--workdir", "WORKDIR",
    "The directory you want pm to do its work in, where 'work' is any file " \
    "copying, downloading, etc. Roughly any scratch space pm needs to build " \
  ...
  PM::Package.types.each do |name, klass|
  ...
  def execute
    # Short-circuit if someone simply runs `pm --version`
    if ARGV == [ "--version" ]
      puts PM::VERSION
      return 0
    end
    ...
    #   'pm -s dir -t ... -C somepath'
    # pm would assume you meant to add '.' to the end of the commandline.
    ...
    input_class = PM::Package.types[input_type]
    output_class = PM::Package.types[output_type]
    ...
    self.class.declared_options.each do |option|
      option.attribute_name.tap do |attr|
        ...
        # attributes hash. (See PM::Package#attributes)
        #
    ...
    rescue PM::Package::FileAlreadyExists => e
      logger.fatal(e.message)
      return 1
    rescue PM::Package::ParentDirectoryMissing => e
      logger.fatal(e.message)
      return 1
    end
  ...
  def run(*run_args)
    ...
    # pm initialization files, note the order of the following array is
    # important, try .pm in users home directory first and then the current
    # directory
    rc_files = [ ".pm" ]
    rc_files << File.join(ENV["HOME"], ".pm") if ENV["HOME"]

    rc_args = []

    if ENV["PMOPTS"]
      logger.warn("Loading flags from PMOPTS environment variable")
      rc_args.push(*Shellwords.shellsplit(ENV["PMOPTS"]))
    end
    ...
    ARGV.push(*args)
    super(*run_args)
  rescue PM::Package::InvalidArgument => e
  ...
  class Validator
    ...
    def validate
      ...
      types = PM::Package.types.keys.sort
      @command.input_type.tap do |val|
        next if val.nil?
        mandatory(PM::Package.types.include?(val),
